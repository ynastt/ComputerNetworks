...
type Client struct {
	...
	first    *big.Int      // 1 элемент
	second    *big.Int      // 2 элемент
...
func (client *Client) handleRequest(req *proto.Request) bool {
	...
	case "add":
		...
		if req.Data == nil {
			...
			if err := json.Unmarshal(*req.Data, &pair); err != nil {
				...
				var x, y big.Int
				if _, ok := x.SetString(pair.Num1, 10); !ok {
					errorMsg = "malformed data field"
				} else if   _, ok := y.SetString(pair.Num2, 10); !ok {
					errorMsg = "malformed data field"
				} else {
					client.logger.Info("performing addition", "value1", x.String())
					client.logger.Info("performing addition", "value2", y.String())
					client.first.Add(client.first, &x)
					client.second.Add(client.second, &y)
					client.count++
				}
			}
		}
	...
	case "avg":
		if client.count > 1 {
			...
			var avg, bvg, cvg big.Int
			avg.Mul(client.first, big.NewInt(1))
			bvg.Mul(client.second, big.NewInt(1))
			cvg.Mul(client.first, client.second)
			client.respond("result", &proto.Pair{
				Num1:   avg.GCD(nil,nil,client.first, client.second).String(),
				Num2: 	cvg.Div(&cvg, avg.GCD(nil,nil,client.first, client.second)).String(),
			})
		}
	default:
