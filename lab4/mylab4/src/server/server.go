...
type Client struct {
	...
	v    	int      		// Скорость камня
	alp   	int      		// Угол, под которым брошен камень
	count   int         	// Посчитанная высота
}
...
func handleRequest(req *proto.Request,conn *net.UDPConn, addr *net.UDPAddr, mapc map[string]*Client) bool {
	...
	case "height":
		...
		h := strconv.Itoa(mapc[clientAddrStr].count)
		var raw json.RawMessage
	...
	case "add": 
		...
		if req.Data == nil {
			...
			if err := json.Unmarshal(*req.Data, &elem); err != nil {
				...
				if vel, err := strconv.Atoi(elem.Velocity); err != nil {
					...
					if alpha, err := strconv.Atoi(elem.Angle); err != nil {
						...
						num := float64(vel * vel) * math.Sin(fi) * math.Sin(fi)
						den:= float64(2 * g)
						help = int(num / den)
